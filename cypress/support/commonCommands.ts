...
class CommonCommands {
    ...
    static inputTextAndRetryIfFail(locator: string, expectedText: string) {
        cy.xpath(locator).clear().type(expectedText);
        cy.xpath(locator,{ timeout: 1000 }).wait(500).invoke('val').then((inputText) => {
            cy.log("expectedText:",expectedText,"\ninputText",inputText)
            if(inputText!=expectedText){
                CommonCommands.inputTextAndRetryIfFail(locator,expectedText);
            }
        })
    }
}
